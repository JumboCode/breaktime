...
const CarouselItem = ({ service }) => {
    ...
    const [popupMode, setPopupMode] = useState(null); // null | 'guidelines' | 'booking'
    ...
    return (
        ...
        >
            ...
            <p
                className={`${isHovering ? 'underline' : ''} font-bold text-lg`}
                onClick={() => setPopupMode('guidelines')}
            >
        ...
        {popupMode && createPortal(
            <LandingPagePopup
                ...
                service={service}
                initialIsBooking={popupMode === 'booking'}
            />,
            document.body
        )}
        </>
