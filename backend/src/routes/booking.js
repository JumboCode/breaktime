    ...
    try {
        ...
        if (!status && !duration && !clientName && !serviceID) {
            return res.status(400).json({
                success: false,
                message: 'Validation error',
                error: 'At least one field (status, duration, clientName, or serviceID) must be provided for update'
            });
        }
...
router.delete('/delete', async (req, res) => {
    try {
        ...
        const deletedBooking = await bookingsCollection.findOneAndDelete({
            bookingID: bookingIDNum
        });
